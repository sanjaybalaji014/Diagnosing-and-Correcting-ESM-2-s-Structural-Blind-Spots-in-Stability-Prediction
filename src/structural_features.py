...
def compute_position_offset(residue_table: list, sequence, min_match_len: int = 8):
...
def get_conservation_score(protein_id: str, resnum: int, consurf_lookup=None) -> float:
...
def prepare_protein_structure(
    ...
    if offset is None:
        offset = 0
        if sequence:
            print(f"[WARN] Could not align sequence to structure for {protein_id} - using position as-is.")
    elif offset != 0:
        print(f"[INFO] {protein_id}: structure residue numbering offset by {offset:+d} relative to mutation positions.")
